@php
    $pageTitle = 'Materi 3: Nutrisi dan Transport';
@endphp
<div class="bg-purple text-white">
    <div class="position-relative h-100 w-100">
        <div class="p-5 h-100 d-flex flex-column">
            <div>
                <div class="row">
                    <div class="col-9">
                        <h6 class="bg-white rounded text-purple px-3 py-1">
                            3: Explanation & Evaluation
                        </h6>
                        Komparasi data dari berbagai sumber
                    </div>
                </div>
                <div class="mt-3">
                    <div class="fw-bold">
                        Analisis hubungan antara air, ion, dan membran
                    </div>
                    <a class="d-block ms-3 text-white mt-1" href="#" style="text-decoration: none;">
                        <i class="bi bi-file-earmark-fill"></i> LKM 14-15: Infografis solusi + refleksi akhir topik
                    </a>
                </div>
            </div>

            <div class="mt-5">
                <div class="d-flex justify-content-end">
                    <a class="btn btn-light text-purple rounded px-2 py-0" href="#" style="text-decoration: none;">
                        <div class="d-flex align-items-center flex-column text-center">
                            <div>
                                <i class="bi bi-file-earmark-check fs-3"></i>
                            </div>
                            <div class="ms-2">
                                Pengumpulan Tugas
                            </div>
                        </div>
                    </a>
                </div>
            </div>
        </div>
        <div class="position-absolute" style="bottom:12px; left: 16px;">
            <small>{{$pageTitle}}</small>
        </div>
    </div>
</div>